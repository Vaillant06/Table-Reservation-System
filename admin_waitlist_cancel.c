...
int main() {
    ...
    char d_name[100], d_phone[50], d_date[50], d_time[50], d_vip[10], d_people[300];
    ...
    // Modify this to read and remove waitlist entries
    FILE *file = fopen("waitlist.txt", "r");
    FILE *temp = fopen("temp.txt", "w");
    if (!file || !temp) {
        printf("<h2>Error opening files</h2>");
        return 1;
    }
    ...
    printf(".message-box h2 { margin-bottom: 20px; color: %s; }", found ? "#28a745" : "#dc3545"); // Green if success, red if fail
    ...
    if (found) {
        printf("<h2>Waitlist entry canceled successfully!</h2>");
    } else {
        printf("<h2>Waitlist entry not found!</h2>");
    }
